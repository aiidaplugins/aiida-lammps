...
from aiida.orm.data.parameter import (
    ParameterData,  # pylint: disable=no-name-in-module, import-error
)
from aiida.parsers.parser import Parser
from aiida_phonopy.common.raw_parsers import (
    parse_FORCE_CONSTANTS,  # pylint: disable=import-error
)
...
class DynaphopyParser(Parser):
    ...
    def parse_with_retrieved(self, retrieved):
        ...
        try:
            out_folder = retrieved[
                self._calc._get_linkname_retrieved()
        ...
        qp_file = out_folder.get_abs_path(
            self._calc._OUTPUT_QUASIPARTICLES
        )  # pylint: disable=no-member, protected-access
        ...
        warnings = []
        with open(
            out_folder.get_abs_path(self._calc._SCHED_ERROR_FILE)
        ) as _file:  # pylint: disable=no-member, protected-access
            errors = _file.read()
        if errors:
            warnings = [errors]
