...
ParsingResources = namedtuple(
    'ParsingResources',
    ['exit_code', 'sys_paths', 'traj_paths', 'restart_paths'])
...
class LAMMPSBaseParser(Parser):
    ...
    def get_parsing_resources(
        ...
        try:
            ...
            return ParsingResources(self.exit_codes.ERROR_NO_RETRIEVED_FOLDER,
                                    None, None, None)
        ...
        if self.node.get_option('output_filename') not in list_of_files:
            return ParsingResources(self.exit_codes.ERROR_LOG_FILE_MISSING,
                                    None, None, None)

        # check stdin and stdout
        if self.node.get_option('scheduler_stdout') not in list_of_files:
            return ParsingResources(self.exit_codes.ERROR_STDOUT_FILE_MISSING,
                                    None, None, None)
        if self.node.get_option('scheduler_stderr') not in list_of_files:
            return ParsingResources(self.exit_codes.ERROR_STDERR_FILE_MISSING,
                                    None, None, None)
