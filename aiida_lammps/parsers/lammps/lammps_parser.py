...
class LAMMPSBaseParser(Parser):
    ...
    def parse_restartfile(self, parameters: dict, list_of_files: list):
        """
        ...
        if parameters.get("restart", {}).get("print_final", False):
            if restart_filename not in list_of_files:
                restart_found = False
            else:
        ...
        if (
            ...
            temporary_list = self.node.base.attributes.get(
            ...
            restartfiles = [
                ...
                if entry in glob.glob(f"{restart_filename}*")
            ]
