...
def write_potential_block(potential=None, parameters_potential: dict = None) -> str:
    """
...
def join_keywords(value) -> str:
    ...
    :rtype: str
    """
    return " ".join([f"{entry['keyword']} {entry['value']}" if isinstance(entry, dict) else f"{entry}" for entry in value])
