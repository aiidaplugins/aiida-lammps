...
def readnumline(f, vtype):
    ...
    if line[1] != '!': raise Exception(
        'not on a line containing ! (as expected) while reading {0}'.format(vtype))
    return int(line[0])
...
_paramkeys = [
    ...
    'Valency angle/lone pair',
    'Valency angle 1',
    'Valency angle 2',
    'Not used 2',
    'Double bond/angle',
    'Double bond/angle: overcoord 1',
    'Double bond/angle: overcoord 2',
    'Not used 3',
    'Torsion/BO',
    'Torsion overcoordination 1',
    'Torsion overcoordination 2',
    'Not used 4',
    'Conjugation',
    'vdWaals shielding',
    'bond order cutoff',
    'Valency angle conjugation 2',
    'Valency overcoordination 1',
    'Valency overcoordination 2',
    'Valency/lone pair',
    'Not used 5',
    'Not used 6',
    'Not used 7',
    'Not used 8',
    'Valency angle conjugation 3']

_speckeys = ['idx', 'symbol', 'reaxff1_radii1', 'reaxff1_valence1', 'mass', 'reaxff1_morse3',
             'reaxff1_morse2', 'reaxff_gamma', 'reaxff1_radii2', 'reaxff1_valence3',
             'reaxff1_morse1', 'reaxff1_morse4', 'reaxff1_valence4', 'reaxff1_under',
             'dummy1', 'reaxff_chi', 'reaxff_mu', 'dummy2',
             'reaxff1_radii3', 'reaxff1_lonepair2', 'dummy3', 'reaxff1_over2',
             'reaxff1_over1', 'reaxff1_over3', 'dummy4', 'dummy5',
             'reaxff1_over4', 'reaxff1_angle1', 'dummy11', 'reaxff1_valence2',
             'reaxff1_angle2', 'dummy6', 'dummy7', 'dummy8']

_bondkeys = ['idx1', 'idx2', 'reaxff2_bond1', 'reaxff2_bond2', 'reaxff2_bond3',
             'reaxff2_bond4', 'reaxff2_bo5', 'reaxff2_bo7', 'reaxff2_bo6', 'reaxff2_over',
             'reaxff2_bond5', 'reaxff2_bo3', 'reaxff2_bo4', 'dummy1', 'reaxff2_bo1', 'reaxff2_bo2',
             'reaxff2_bo8', 'reaxff2_bo9']

_odkeys = ['idx1', 'idx2', 'reaxff2_morse1', 'reaxff2_morse3', 'reaxff2_morse2',
           'reaxff2_morse4', 'reaxff2_morse5', 'reaxff2_morse6']

_anglekeys = ['idx1', 'idx2', 'idx3', 'reaxff3_angle1', 'reaxff3_angle2', 'reaxff3_angle3', 'reaxff3_conj',
              'reaxff3_angle5', 'reaxff3_penalty', 'reaxff3_angle4']

_torkeys = ['idx1', 'idx2', 'idx3', 'idx4',
...
def read_reaxff_file(inpath):
    with open(inpath, 'r') as f:
        ...
        if npar != len(_paramkeys):
            raise IOError('Expecting {} general parameters'.format(len(_paramkeys)))
        reaxff_par = {}
        ...
        for i in range(nspec):
            ...
            spec_values.append([idx, symbol] + [float(v) for v in values] + readlist(f) + readlist(f) + readlist(f))
            if len(spec_values[i]) != len(_speckeys):
                raise Exception('number of values different than expected for species {0}'.format(symbol))
        ...
        spec_dict['reaxff1_lonepair1'] = (0.5 * (np.array(spec_dict["reaxff1_valence3"])
                                                 - np.array(spec_dict["reaxff1_valence1"]))).tolist()
        ...
        for i in range(nbond):
            ...
            if len(bond_values[i]) != len(_bondkeys):
                raise Exception('number of values different than expected for bond')
        ...
        for i in range(nod):
            ...
            if len(od_values[i]) != len(_odkeys):
                raise Exception('number of values different than expected for off-diagonal')
        ...
        for i in range(nangle):
            ...
            if len(angle_values[i]) != len(_anglekeys):
                raise Exception('number of values different than expected for angle')
        ...
        for i in range(ntors):
            ...
            if len(torsion_values[i]) != len(_torkeys):
                raise Exception('number of values different than expected for torsion')
        ...
        for i in range(nhb):
            ...
            if len(hbond_values[i]) != len(_hbkeys):
                raise Exception(
                    'number of values different than expected for hbond {0},{1},{2}'.format(species1, species2,
                                                                                            species3))
        ...
        return {
            ...
            "off-diagonals": od_dict,  # od_df.reset_index().to_dict(orient='list'),
            "hbonds": hbond_dict,  # hbond_df.reset_index().to_dict(orient='list'),
            "torsions": torsion_dict,  # torsion_df.reset_index().to_dict(orient='list'),
            "angles": angle_dict,  # angle_df.reset_index().to_dict(orient='list')
        }
...
def write_lammps(data):
    ...
    spec_data = transpose([data['species'][key] for key in _speckeys if key != 'idx'])
...
def write_gulp(data, species_filter=None):
    ...
    hbond_df['idxs'] = zip(hbond_df['idx2'], hbond_df['idx1'], hbond_df['idx3'])
    angle_df = data["angles"]
    angle_df['idxs'] = zip(angle_df['idx2'], angle_df['idx1'], angle_df['idx3'])
    torsion_df = data["torsions"]
    torsion_df['idxs'] = zip(torsion_df['idx1'], torsion_df['idx2'], torsion_df['idx3'], torsion_df['idx4'])
    ...
    bond_df = [AttrDict({k: v[i] for k, v in bond_df.items()})
               for i in range(len(bond_df['idxs']))]
    od_df = [AttrDict({k: v[i] for k, v in od_df.items()})
             for i in range(len(od_df['idxs']))]
    hbond_df = [AttrDict({k: v[i] for k, v in hbond_df.items()})
                for i in range(len(hbond_df['idxs']))]
    angle_df = [AttrDict({k: v[i] for k, v in angle_df.items()})
                for i in range(len(angle_df['idxs']))]
    torsion_df = [AttrDict({k: v[i] for k, v in torsion_df.items()})
                  for i in range(len(torsion_df['idxs']))]
    ...
    def write_data(name, df, wdata, conditions=None, extra_data=None):
        ...
        for s in df:
            ...
            evals = [True for c in conditions if eval(c)]
            ...
            try:
                ...
                raise Exception('ERROR: Species number out of bounds when getting {}'.format(name))
            ...
            lines.append(regex.format(*symbols + [s[w] for w in wdata] + extra_data))
    ...
    outstr += (
        "reaxff0_valence  {:12.6f} {:12.6f} {:12.6f} {:12.6f}\n".format(reaxff_par['Valency undercoordination'],
                                                                        reaxff_par['Valency/lone pair'],
                                                                        reaxff_par['Valency angle 1'],
                                                                        reaxff_par['Valency angle 2']))
    outstr += ("reaxff0_penalty  {:12.6f} {:12.6f} {:12.6f}\n".format(reaxff_par['Double bond/angle'],
                                                                      reaxff_par['Double bond/angle: overcoord 1'],
                                                                      reaxff_par['Double bond/angle: overcoord 2']))
    outstr += (
        "reaxff0_torsion  {:12.6f} {:12.6f} {:12.6f} {:12.6f}\n".format(reaxff_par['Torsion/BO'],
                                                                        reaxff_par['Torsion overcoordination 1'],
                                                                        reaxff_par['Torsion overcoordination 2'],
                                                                        reaxff_par['Conjugation']))
    outstr += "reaxff0_vdw      {:12.6f}\n".format(reaxff_par['vdWaals shielding'])
    outstr += "reaxff0_lonepair {:12.6f}\n".format(reaxff_par['Valency angle/lone pair'])
    ...
    outstr += write_data('reaxff1_lonepair kcal', spec_df, ['reaxff1_lonepair1', 'reaxff1_lonepair2'])
    outstr += write_data('reaxff1_angle', spec_df, ['reaxff1_angle1', 'reaxff1_angle2'])
    ...
    outstr += write_data('reaxff2_pen kcal', bond_df,
                         ['reaxff2_bo9'], ['s.reaxff2_bo9>0.0'],
    ...
    outstr += write_data('reaxff3_conjugation kcal', angle_df,
                         ...
                         [reaxff_par['Valency angle conjugation 1'], reaxff_par['Valency angle conjugation 3'],
