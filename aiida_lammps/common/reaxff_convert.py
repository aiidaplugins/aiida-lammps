...
def read_lammps_format(lines, tolerances=None):
    ...
    if lines[lineno].split()[0] != str(len(KEYS_GLOBAL)):
        raise IOError('Expecting {} global parameters'.format(
            len(KEYS_GLOBAL)))
    ...
    output['global'][
        'reaxff2_pen3'] = 1.0  # this is not provided by lammps, but is used by GULP

    tolerances = tolerances or {}
    output['global'].update(
        {k: tolerances.get(k, v)
         for k, v in DEFAULT_TOLERANCES})
    ...
    for _ in range(num_lines):
        values = split_numbers(lines[lineno]) + split_numbers(
            lines[lineno + 1])
        ...
        if len(values) != len(KEYS_2BODY_BONDS):
            raise Exception(
                ...
                '{1} != {2}'.format(key_name, len(values),
                                    len(KEYS_2BODY_BONDS)))
    ...
    for _ in range(num_lines):
        ...
        if len(values) != len(KEYS_2BODY_OFFDIAG):
            raise Exception(
                ...
                '{2} != {3}'.format(key_name, lineno - 1, len(values),
                                    len(KEYS_2BODY_OFFDIAG)))
...
def filter_by_species(data, species):
    ...
    def convert_indices(key):
        return INDEX_SEP.join([
            str(species.index(data['species'][int(k)]))
            for k in key.split(INDEX_SEP)
        ])
